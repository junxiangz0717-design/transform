...
class Node_RADICAL : public SyncActionNode 
{
public:
    Node_RADICAL(const string &name, const NodeConfig &config) : SyncActionNode(name, config) 
    {charge_timer.reset();defence_timer.reset();}
    
    ~Node_RADICAL() override = default;
