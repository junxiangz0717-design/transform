...
public:
    ...
    NodeStatus tick() override
    {
        ...
        if (DD.targets.targets_point[DD.targets.autoaim_target]->get_x() != 0 && DD.targets.targets_point[DD.targets.autoaim_target]->get_y() != 0) // 目标未消失
        {
            ...
            target = ER.pixel2ul(ER.find_visible_target(ER.ul2pixel(DD.cur_x, DD.cur_y),
                                                        ER.ul2pixel(DD.targets.targets_point[DD.targets.autoaim_target]->get_x(),
                                                                    DD.targets.targets_point[DD.targets.autoaim_target]->get_y())));
            cout << "(" << target.first << "," << target.second << ")" << endl;
            mes_puber->pursuit_data_.agent_x = ER.ul2pixel(DD.cur_x, DD.cur_y).x;
            mes_puber->pursuit_data_.agent_y = ER.ul2pixel(DD.cur_x, DD.cur_y).y;
            mes_puber->pursuit_data_.enemy_x = ER.ul2pixel(DD.targets.targets_point[DD.targets.autoaim_target]->get_x(), DD.targets.targets_point[DD.targets.autoaim_target]->get_y()).x;
            mes_puber->pursuit_data_.enemy_y = ER.ul2pixel(DD.targets.targets_point[DD.targets.autoaim_target]->get_x(), DD.targets.targets_point[DD.targets.autoaim_target]->get_y()).y;
            mes_puber->pursuit_data_.target_x = ER.find_visible_target(ER.ul2pixel(DD.cur_x, DD.cur_y), ER.ul2pixel(DD.targets.targets_point[DD.targets.autoaim_target]->get_x(),
                                                                                                                    DD.targets.targets_point[DD.targets.autoaim_target]->get_y()))
                                                    .x;
            mes_puber->pursuit_data_.target_y = ER.find_visible_target(ER.ul2pixel(DD.cur_x, DD.cur_y), ER.ul2pixel(DD.targets.targets_point[DD.targets.autoaim_target]->get_x(),
                                                                                                                    DD.targets.targets_point[DD.targets.autoaim_target]->get_y()))
                                                    .y;
