...
class MoveToPoint_once: public StatefulActionNode{
private:
...
public:
    ...
    MoveToPoint_once(const string& name, const NodeConfig& config): StatefulActionNode(name, config){ }

    ~MoveToPoint_once() override = default;
    ...
    NodeStatus onStart() override
    {
        ...
        current_index += 1;
        if(static_cast<size_t>(current_index) > points.size()){DD.is_start = 0;return NodeStatus::SUCCESS;}
