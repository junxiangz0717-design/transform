...
private:
    int restart_count = 0;
    AMMO ammo_state = AMMO::HIGH_AMMO;
...
public:
    ...
    static PortsList providedPorts()
    {
        return {
            ...
            InputPort<AMMO>("ammo_state", "当前弹药状态")};
    }
    ...
    NodeStatus tick() override
    {
        ...
        if (DD.time <= 180)
        {
            DD.target_list[0][2] = 10;
            DD.target_list[1][2] = 10;
            DD.target_list[2][2] = 10;
        }
        switch (ammo_state)
        {
        case NO_AMMO:
    ...
    std::pair<Target, double> BestTarget(const vector<Target> &targets_vector, const vector<double> &targets_distance_vector, int if_detec)
    {
        ...
        for (const auto &target : targets_vector)
        {
            ...
            if ((DD.target_list[DD.priority][target_index] == 0) || (target_index == 5 && if_detec != 1))
            {
                continue;
            }
            ...
            if (distance >= 7.0)
                continue;
            else if (distance < 7.0 && distance >= 5.5)
                score += 300;
            else
            {
                score += 600;
