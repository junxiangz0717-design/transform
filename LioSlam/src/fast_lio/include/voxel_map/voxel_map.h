...
// 1. 添加 ROS 2 IMU 消息头文件
#include <sensor_msgs/msg/imu.hpp>
#include <deque>
#include "use-ikfom.hpp"
#include "UniversalLogger.hpp"
// 2. 定义 ImuConstPtr 类型别名
// (如果之前没定义过，必须加这一行，编译器才知道 ImuConstPtr 是什么)
...
// ros::NodeHandle 替换为 std::shared_ptr<rclcpp::Node>
...
public:
  ...
  // ros::Publisher 替换为 rclcpp::Publisher<Type>::SharedPtr
...
// 只需要传入一个向量：单位重力向量
void Generate2DImage(cv::Mat& img, 
                     const Eigen::Vector3d& center_pos, 
                     double range, 
                     double resolution,
                     const Eigen::Vector3d& g_direction, // 新增：重力单位向量
                     pcl::PointCloud<pcl::PointXYZINormal>::Ptr debug_cloud_out=nullptr);
  ...
  // geometry_msgs::Quaternion 替换为 geometry_msgs::msg::Quaternion
...
private:
  ...
  // visualization_msgs::MarkerArray 替换为 visualization_msgs::msg::MarkerArray
