...
void loadVoxelConfig(std::shared_ptr<rclcpp::Node> nh, VoxelMapConfig &voxel_config)
{
  ...
  voxel_config.sigma_num_ = nh->declare_parameter<double>("lio/sigma_num", 3.0);
  ...
  // Handling vector<int> (ROS 2 uses vector<int64_t> for parameters)
  std::vector<int64_t> default_layer_init = {5, 5, 5, 5, 5};
  std::vector<int64_t> param_layer_init = nh->declare_parameter<std::vector<int64_t>>("lio/layer_init_num", default_layer_init);
  voxel_config.layer_init_num_.resize(param_layer_init.size());
  for(size_t i=0; i<param_layer_init.size(); ++i) {
      voxel_config.layer_init_num_[i] = static_cast<int>(param_layer_init[i]);
  }
  ...
  voxel_config.sliding_thresh = nh->declare_parameter<double>("local_map/sliding_thresh", 8.0);
}
...
void VoxelMapManager::StateEstimation(StatesGroup &state_propagat)
{
  ...
  for (int iterCount = 0; iterCount < config_setting_.max_iterations_; iterCount++)
  {
    ...
    if (!EKF_stop_flg && (rematch_num >= 2 || (iterCount == config_setting_.max_iterations_ - 1)))
    {
      ...
      // ROS 2 replacement for tf::createQuaternionMsgFromRollPitchYaw
      tf2::Quaternion q;
      // ================= [修复开始] =================
      // 1. 将当前的旋转矩阵直接转换成 Eigen 四元数
      // 这种转换在数学上是全向稳定的，不存在万向节死锁（Gimbal Lock）
      Eigen::Quaterniond q_eigen(state_.rot_end);
      q_eigen.normalize(); // 归一化，防止长时间运行后的数值漂移

      // 2. 直接将四元数的 x,y,z,w 赋值给 tf 的 q 对象
      // 注意：tf::Quaternion 的 setValue 顺序通常是 x, y, z, w
                                     ...
                                     const float alpha, const Eigen::Vector3d rgb)
{
  ...
  // ROS 2 time retrieval without node ptr, assumes wall time for visualization
  ...
  plane.color.g = rgb(1);
  plane.color.b = rgb(2);
  // ROS 2 Duration (sec, nanosec), 0 means infinite
...
void VoxelMapManager::mapSliding()
{
  ...
  {
    std::cout<<"[DEBUG]: Last sliding length "<<(position_last_ - last_slide_position).norm()<<"\n";
    return;
  }
  ...
  std::cout<<"[DEBUG]: Map sliding using "<<t_sliding_end - t_sliding_start<<" secs"<<"\n";
  return;
}
...
void VoxelMapManager::Generate2DImage(cv::Mat& img, 
                                      const Eigen::Vector3d& center_pos, 
                                      double range, 
                                      double resolution,
                                      const Eigen::Vector3d& g_direction, 
                                      pcl::PointCloud<pcl::PointXYZINormal>::Ptr debug_cloud_out)
{
    // [诊断变量]
    int cnt_voxels = 0;
    int cnt_pass = 0;
    ...
    int center_pixel = size / 2;
    img = cv::Mat::zeros(size, size, CV_8UC1);

    // 这里的 n_vec 必须指向物理天空
    // state.grav (g_direction) 指向地心，所以取反
    Eigen::Vector3d n_vec = -g_direction.normalized(); 
    ULOG_PLOT("Map_Gen_Debug", "n_vec_z", n_vec.z());

    // 构建投影基底 (简单版，假设X轴大致水平)
    Eigen::Vector3d z_w = n_vec;
    ...
    for (const auto& kv : voxel_map_) {
        auto octo_tree = kv.second;
        if (octo_tree == nullptr) continue;
        cnt_voxels++;

        Eigen::Vector3d p_world(octo_tree->voxel_center_[0], octo_tree->voxel_center_[1], octo_tree->voxel_center_[2]);
        Eigen::Vector3d diff_vec = p_world - center_pos;
        
        // 计算相对物理高度
        double rel_height = diff_vec.dot(n_vec); 
        
        if (rel_height < min_h) min_h = rel_height;
        if (rel_height > max_h) max_h = rel_height;
        if (rel_height < 0.8 || rel_height > 1.8) continue;

        // 投影到水平面
        double diff_x = diff_vec.dot(x_w);
        double diff_y = diff_vec.dot(y_w);

        if (std::abs(diff_x) > range || std::abs(diff_y) > range) continue;

        int u = std::round((diff_x / resolution) + center_pixel);
        int v = std::round((diff_y / resolution) + center_pixel);

        if (u >= 0 && u < img.cols && v >= 0 && v < img.rows) {
            img.at<uchar>(v, u) = 255;
            cnt_pass++;
            if (debug_cloud_out) {
                pcl::PointXYZINormal pt; pt.x=p_world.x(); pt.y=p_world.y(); pt.z=p_world.z(); pt.intensity=rel_height;
                debug_cloud_out->push_back(pt);
            }
        }
    }

    ULOG_PLOT("Map_Gen_Debug", "cnt_voxels", (double)cnt_voxels);
    ULOG_PLOT("Map_Gen_Debug", "cnt_pass", (double)cnt_pass);
    ULOG_PLOT("Map_Gen_Debug", "min_h", min_h);
    ULOG_PLOT("Map_Gen_Debug", "max_h", max_h);
}
